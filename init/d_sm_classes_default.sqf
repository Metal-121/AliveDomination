...
d_sm_apc = switch (d_enemy_side_short) do {
	case "E": {["O_SerbienArmy_BMP_2D_WDL_01","O_SerbienArmy_BTR60PB_WDL_01"] select (d_tanoa || {d_livonia})};
...
d_sm_cargotrucktype = switch (d_enemy_side_short) do {
	case "E": {["O_SerbienArmy_M1152_Cargo_WDL_01","O_SerbienArmy_Ural_375D_open_WDL_01"] select (d_tanoa || {d_livonia})};
	case "W": {"B_Truck_01_covered_F"};
	case "G": {"I_Truck_02_covered_F"};
};
d_sm_fueltrucktype = switch (d_enemy_side_short) do {
	case "E": {["O_SerbienArmy_Ural_375D_Fuel_WDL_01","O_SerbienArmy_Ural_375D_Fuel_WDL_01"] select (d_tanoa || {d_livonia})};
	case "W": {"B_Truck_01_fuel_F"};
	case "G": {"I_Truck_02_fuel_F"};
};
d_sm_camo_net = call {
   if (d_enemy_side_short == "W") exitWith {"CamoNet_wdl_big_F"};
   if (d_enemy_side_short == "E") exitWith {"CamoNet_wdl_big_F"};
   "CamoNet_INDP_big_F"
};
d_sm_medtrucktype = switch (d_enemy_side_short) do {
	case "E": {"O_SerbienArmy_Van_Ambulance_WDL_01"};
	case "W": {"B_Truck_01_medical_F"};
	case "G": {"I_Truck_02_medical_F"};
};
d_sm_deliver_truck = switch (d_enemy_side_short) do {
	case "E": {["sfp_tgb20", "sfp_tgb1317", "sfp_tgb1314", "sfp_tgb13_ksp58"]};
	case "W": {["O_SerbienArmy_URAL_4320_Repair_WDL_01", "O_SerbienArmy_M1152_RSV_WDL_01", "O_SerbienArmy_Ural_375D_Fuel_WDL_01", "O_SerbienArmy_Van_Ambulance_WDL_01"]};
