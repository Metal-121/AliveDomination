...
d_x_drop_array =
	...
	call {
		...
		[[], [localize "STR_DOM_MISSIONSTRING_22", ["sfp_tgb16_rws", "sfp_tgb16","sfp_patgb360"] select (d_tanoa || {d_livonia})], [localize "STR_DOM_MISSIONSTRING_20", "sfp_ammobox_container_2015"]]
	};
#endif
#ifdef __OWN_SIDE_OPFOR__
	call {
...
d_flag_str_blufor = "pics\sweden_flag.paa";
...
// artillery operators
#ifndef __TT__
d_can_use_artillery = ["d_artop_1", "d_artop_2","d_commander"]; // case has to be the same as in mission.sqm, d_artop_1 D_ARTOP_1 is not the same :)
#else
...
#ifndef __TT__
d_can_mark_artillery = ["d_alpha_1", "d_bravo_1", "d_charlie_1", "d_echo_1","d_commander"];
#else
d_can_mark_artillery = ["d_blufor_1", "d_blufor_2", "d_blufor_3", "d_opfor_1", "d_opfor_2", "d_opfor_3"];
#endif

#ifndef __TT__
d_can_call_cas = ["d_alpha_1", "d_bravo_1", "d_charlie_1", "d_echo_1","d_commander"];
#else
...
if (isNil "d_cas_available_time") then {
	d_cas_available_time = 90; // time till CAS is available again!
};
...
call {
	if (d_own_side == "WEST") exitWith {
		d_the_box = "sfp_ammobox_container_2015";
		d_the_base_box = "sfp_ammobox_container_2015";
	};
...
#include "d_specops_O_uns.sqf"
	...
	d_specops_W = call {
		...
		[["West","sfp_swe_2015","Infantry","SpecOps","BSWE15_RecSquad"] call d_fnc_GetConfigGroup,["West","dfp_swe_2015","Infantry","SpecOps","BSWE15_RecTeam"] call d_fnc_GetConfigGroup]
	};

	if (d_tanoa || {d_livonia}) then {
		d_sniper_E = [["rhssaf_army_o_m10_para_sniper_m82a1","rhssaf_army_o_m93_oakleaf_summer_sniper_m76","rhssaf_army_o_m93_oakleaf_summer_spotter"],["O_T_ghillie_tna_F","O_T_ghillie_tna_F"],["O_ghillie_lsh_F","O_ghillie_lsh_F"]];
		d_sniper_W = [["West","sfp_swe_2015","Infantry","SpecOps","BSWE15_RecSniperTeam"] call d_fnc_GetConfigGroup,["B_T_ghillie_tna_F","B_T_ghillie_tna_F"],["B_ghillie_lsh_F","B_ghillie_lsh_F"]];
		...
		d_sniper_W = [["West","sfp_swe_2015","Infantry","SpecOps","BSWE15_RecSniperTeam"] call d_fnc_GetConfigGroup,["B_ghillie_ard_F","B_ghillie_ard_F"],["B_ghillie_lsh_F","B_ghillie_lsh_F"],["B_ghillie_sard_F","B_ghillie_sard_F"]];
		d_sniper_G = [["Indep","IND_F","Infantry","HAF_SniperTeam"] call d_fnc_GetConfigGroup];
	};
	...
	d_sabotage_E = [["rhssaf_army_o_m93_oakleaf_summer_exp"]];
	d_sabotage_W = [["sfp_m90w_sog_explosive_specialist"]];
...
#endif
#ifdef __TANOA__
	d_arti_observer_E = [["rhssaf_army_o_m93_oakleaf_summer_spotter"]];
#endif
#ifdef __STRATIS__
	d_arti_observer_E = [["O_recon_JTAC_F"]];
#endif
...
#endif
#ifndef __IFA3LITE__
	d_arti_observer_W = [["sfp_m90w_sog_jtac"]];
#else
...
//#include "d_sm_classes_csla.sqf"
	...
	call {
		if (d_enemy_side_short == "E") exitWith {
			d_air_radar = "rhs_prv13_turret_vpvo";
			d_air_radar2 = "rhs_p37_turret_vpvo";
	...
	d_airai_attack_plane = call {
		if (d_enemy_side_short == "E") exitWith {
			call {
				...
				["rhssaf_airforce_o_l_18_101","rhssaf_airforce_o_l_18"]
			};
		};
		if (d_enemy_side_short == "W") exitWith {
			call {
...
/*#ifdef __ALTIS__
	...
	if (isNil "d_ai_groups_respawn_time") then {
		...
		d_ai_groups_respawn_time = [240, 150, 320, 170];
	};
...
if (hasInterface) then {
	...
	if (isNil "d_reserved_slot") then {
		d_reserved_slot = ["d_commander","d_admin","d_admin","d_admin","d_admin"];
	};
	...
	if (isNil "d_uid_reserved_slots") then {
		d_uid_reserved_slots = ["76561198042051554"];
		d_uids_for_reserved_slots = ["76561198042051554","76561198042051554"];
	};
	...
	call {
		if (d_tanoa || {d_livonia}) exitWith {
			["sfp_cykel42","B_Quadbike_01_F", "sfp_tgb1111_sog_ksp58"]
		};
		if (d_gmcwg) exitWith {
			if (d_gmcwgwinter) exitWith {
	...
	"uns_M113_transport","uns_UH1H_m60","gm_ge_army_bo105m_vbh","sfp_strb90","sfp_hkp9","sfp_hkp4_2015","sfp_hkp16","sfp_bv206","sfp_tgb20"];
#endif
#ifdef __OWN_SIDE_OPFOR__
	call {
		if (d_ifa3lite) exitWith {
	...
	// is engineer
#ifndef __TT__
	d_is_engineer = ["d_delta_1","d_delta_2","d_delta_3","d_delta_4","d_delta_5","d_delta_6","d_commander"];
#else
...
#ifndef __TT__
	d_can_call_drop_ar = ["d_alpha_1","d_charlie_1","d_echo_1","d_commander"];
#else
	d_can_call_drop_ar = [];
#endif
	...
	d_taxi_aircrafts =
		...
		call {
			...
			["sfp_hkp4_2015", "sfp_hkp16", "sfp_hkp9_sog","B_T_VTOL_01_infantry_F"]
		};
#endif
#ifdef __OWN_SIDE_OPFOR__
		call {
